...
if (isset($_GET['action']) && $_GET['action'] == 'press') {
    ...
    $query_pro = "SELECT t.date_time, m.message_patient, m.diagnosis, m.date_time_d, d.first_name, d.last_name
             FROM testResult t, medicalCase m, doctorUsers d WHERE t.id = m.id_testResult AND 
             m.id = '$id_mC' AND m.id_profileDoctor = d.id";
